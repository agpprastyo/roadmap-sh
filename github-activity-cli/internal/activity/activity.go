...
type Fetcher interface {
	FetchGitHubActivity(url, username string) ([]GitHubActivity, error)
}

func FetchGitHubActivity(baseURL, username string) ([]GitHubActivity, error) {
	response, err := http.Get(fmt.Sprintf("%s/users/%s/events", baseURL, username))
	if err != nil {
		return nil, err
	}
